...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #tomap is to make heatmaps
        self.tomap = out + identity
        out += identity
        out = nn.functional.relu(out)
        return out
...
class RepresentationNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.resblocks:
            x = block(x)
        #this internal variable is just used to record the input to a heatmap evaluation
        self.tomap = self.resblocks[-1].tomap
        return x
...
class EfficientZeroNetH(BaseNet):
    def __init__(
        self,
        observation_shape,
        action_space_size,
        ...
        """
        super(EfficientZeroNetH, self).__init__(inverse_value_transform, inverse_reward_transform, lstm_hidden_size)
    ...
    def heatmap(self, layer, f_offset, f_weight):
        ...
        val = heatmap_fun(self.representation_network.tomap).detach().cpu().numpy()

        #take jacobian of heatmap_fun, at point tomap
        #jacobian size is (output test episodes, input test episodes, 64 channels, 6 x, 6 y)
        jacobian = torch.autograd.functional.jacobian(heatmap_fun, self.representation_network.tomap).cpu()

        hmap = np.zeros((jacobian.size()[1], jacobian.size()[3], jacobian.size()[4]))

        #sum (diagonal in episodes) across channels and then cut off negative (inhibitory / cross-talk) part with a relu
        for n in range(jacobian.size()[1]):
            hmap[n] = nn.functional.relu(torch.sum(jacobian[n,n],0)).numpy()
